...
class SampleClientController extends AbstractActionController
{
    public function indexAction()
    {
        ...
        $client->setUri('http://localhost:80'.$this->getRequest()->getBaseUrl().'/kristins-rest');
        ...
        switch($method) {
            ...
            case 'get-list' :
                $client->setMethod('GET');
                $client->setParameterGET(array('ServiceTicketNo'=>'SEO-13-00001'));
                break;
            case 'create' :
                $client->setMethod('POST');
                $client->setParameterPOST(array('SalesMan'=>'0001002'));   // set all data in post 
                break;
            case 'update' :
                $data = array('SalesMan'=>'0001002');        // set all data 
                $adapter = $client->getAdapter();

                $adapter->connect('localhost', 80);
                $uri = $client->getUri().'?ServiceTicketNo=SEO-13-00001';
